...
# <summary>■解答例の表示（Click）</summary><div>
#     
# 補足：常用対数は底が10の対数です。$ \log_{10} x $　計測値の対数表示などに使います。<br>
...
# 0.3010299956639812  
# と表示される。
# また、常用対数は底が10なので、np.log10(100)で2.0と表示される。
#     
# </div></details>
...
# <summary>■解答例の表示（Click）</summary><div>
#     
# 解答例を以下に示す。複数の値に対応した計算結果が欲しい場合、numpyのブロードキャストを利用すると便利だ。また、計算そのものは作った関数に行わせる。ndarray型の変数vがあれば、f(v)でvの要素ごとに関数fの結果が格納された配列が作成される。  
#     
# ```python  
# import numpy as np
# 
