...
# ---
# ### 問題　1つの図に2つのグラフを描画
# そこで、$ -5 < x < 5 $の範囲において、以下の2式を滑らかに描画せよ。グリッドと軸名を表示し、凡例に式番号を表示すること。
# 
# $$
# \begin{align}
#   y &= 2x^3 - 4x + 2   \tag{1} \\
...
# 同じ図の中に2つ以上の折れ線グラフを描画するには、showを呼び出す前に必要なだけplotを呼び出せばよい。
# 解答例を以下に示す。
# 以下の例では、plot(a, b)とplot(a, c)でa, bの組で描画できる線と、a, bの組で描画できる線の2種類の線が描かれる。
#     
# ```python  
# import numpy as np    # numpyを取り込み、npという名前で使う、という命令
# import matplotlib.pyplot as plt     # matplotlibを取り込む
